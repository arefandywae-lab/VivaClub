...
ตราบาปทางสังคม (Social Stigma) — การถูกมองว่า "บ้า" หรือ "อ่อนแอ" ยังคงเป็นอุปสรรคใหญ่ในสังคมไทย คนจำนวนมากเลือกที่จะเก็บปัญหาไว้คนเดียวแทนที่จะขอความช่วยเหลือ
การกระจายตัวของผู้เชี่ยวชาญที่ไม่เท่าเทียม — จิตแพทย์และนักจิตวิทยาส่วนใหญ่กระจุกตัวอยู่ในกรุงเทพมหานครและเมืองใหญ่ ผู้ที่อยู่ในต่างจังหวัดต้องเดินทางไกลหรือรอคิวนาน
ค่าใช้จ่ายสูง — ค่าปรึกษาจิตแพทย์เอกชนอยู่ที่ 1,500–3,000 บาทต่อครั้ง ซึ่งสูงเกินไปสำหรับหลายคน
การขาดจุดเริ่มต้นที่ปลอดภัย — ผู้ที่ยังไม่แน่ใจว่าตนเองต้องการความช่วยเหลือระดับใด ขาดพื้นที่กลางที่จะเริ่มต้นสำรวจความรู้สึกตัวเองโดยไม่ต้องเปิดเผยตัวตน

ช่องว่างสำคัญที่ยังไม่มีใครแก้ไข: ไม่มีแพลตฟอร์มใดในไทยที่เชื่อมต่อ
 
ชุมชนสนับสนุนแบบ real-time + ไม่เปิดเผยตัวตน กับ 
การ triage อาการอัตโนมัติ และ 
การดูแลทางคลินิกกรณีฉุกเฉิน ไว้ในแอปเดียว
...
สร้างพื้นที่ชุมชน แบบ anonymous สำหรับการพูดคุยเรื่องสุขภาพจิตในรูปแบบ real-time audio
ใช้ PHQ-9 assessment เพื่อ triage ระดับความเสี่ยงของผู้ใช้อัตโนมัติ
เชื่อมต่อผู้ใช้ที่มีความเสี่ยงสูงกับแพทย์ผ่านระบบ SOS ทันที
จัดระบบนัดหมายและการปรึกษาทางวิดีโอระหว่างผู้ใช้และแพทย์
ปกป้องความเป็นส่วนตัวของข้อมูลทางคลินิกด้วย End-to-End Encryption
...
REST API Flow: Flutter → HTTPS → Caddy → Django → PostgreSQL → response
WebSocket Flow: Flutter → WSS → Caddy → Daphne → Redis Channel Layer → Daphne → Flutter
LiveKit Flow: Flutter → LiveKit Signaling → LiveKit RTC transport (UDP) → peer participants
Webhook Flow: LiveKit → HTTP POST → Django → PostgreSQL (update participant_count)
Notification Flow: Django → Redis publish → Daphne subscribe → WebSocket push → Flutter
...
Presentation Layer: รับ Input จากผู้ใช้และส่งเป็น Event ไปยัง BLoC
Business Logic Layer (BLoC): ประมวลผล Event และเปลี่ยนสถานะ (Emit State) กลับไปยัง UI
Data Layer (Repository): ทำหน้าที่ดึงข้อมูลจาก API หรือฐานข้อมูลท้องถิ่น
...
UUID Primary Keys: ทุกตารางใช้ UUID แทน Integer IDs เพื่อป้องกันการสุ่มเดาหมายเลขรายการ (ID Enumeration) ซึ่งเป็นมาตรฐานสำคัญสำหรับข้อมูลสุขภาพที่มีความอ่อนไหวสูง
End-to-End Encrypted Storage (E2EE): ในตาราง OPD_NOTE ระบบจะเก็บเพียงข้อมูลที่ถูกเข้ารหัสแล้ว (Ciphertext) และ IV เท่านั้น โดยไม่มีการเก็บกุญแจถอดรหัสไว้ที่ฝั่งเซิร์ฟเวอร์ เพื่อให้เป็นไปตามข้อกำหนด PDPA
JSONField Flexibility: ใช้โครงสร้างข้อมูลแบบ JSONB ในฟิลด์คำตอบแบบทดสอบ (Assessment Answers) และข้อมูลเสริมของห้องสนทนา เพื่อรองรับการปรับเปลี่ยนรูปแบบข้อมูลในอนาคตโดยไม่ต้องเปลี่ยนโครงสร้างฐานข้อมูล (Schema Migration)
Soft Delete Pattern: สำหรับข้อมูลสำคัญเช่น ห้องสนทนา (Rooms) ระบบจะใช้สถานะ is_active แทนการลบข้อมูลจริง เพื่อให้สามารถตรวจสอบย้อนหลังได้ในกรณีที่มีการรายงานความประพฤติ (Room Reports) เกิดขึ้น
Deterministic Room IDs: ในระบบแชท ระบบจะใช้วิธีการจัดเรียง UUID ของผู้สนทนาและนำมาเชื่อมกันเพื่อสร้าง room_id ที่แน่นอน ทำให้ไม่ว่าใครจะเป็นผู้เริ่มสนทนา ระบบจะระบุไปยังห้องแชทเดียวกันเสมอโดยไม่ต้องสร้างตารางความสัมพันธ์เพิ่มเติม
...
Explicit Consent: ระบบจะขอความยินยอมอย่างชัดเจนก่อนการเก็บข้อมูลสุขภาพ (คะแนน PHQ-9)
Data Minimization: เก็บข้อมูลเท่าที่จำเป็นสำหรับการให้บริการเท่านั้น เช่น ในระบบ SOS จะแสดงเพียงคะแนนความเสี่ยงให้คุณหมอเห็น แทนการแสดงชื่อจริงของคนไข้
Right to Access: ผู้ใช้มีสิทธิ์เข้าถึงและจัดการข้อมูลส่วนตัวของตนเองได้ผ่านหน้าโปรไฟล์ในแอปพลิเคชัน
...
การแก้ไข: เปลี่ยนมาใช้ระบบ Monotonic Clock Pacing โดยการคำนวณเวลาที่ "ควรจะ" ส่งเฟรมเสียงถัดไปจากจุดเริ่มต้นที่แน่นอน และสั่งให้บอทส่งเฟรมเสียงทันทีหากพบว่าความเร็วในการประมวลผลช้ากว่ากำหนด
...
Kroenke K, Spitzer RL, Williams JBW. (2001). The PHQ-9: Validity of a Brief Depression Severity Measure. Journal of General Internal Medicine, 16(9), 606–613.  — Standard PHQ-9 scoring และ cutoff scores ที่ VivaClub ใช้
กรมสุขภาพจิต กระทรวงสาธารณสุข. (2566). รายงานสถานการณ์สุขภาพจิตในประเทศไทย. กรุงเทพฯ: กรมสุขภาพจิต.
World Health Organization. (2022). World Mental Health Report: Transforming Mental Health for All. Geneva: WHO.
พระราชบัญญัติคุ้มครองข้อมูลส่วนบุคคล พ.ศ. 2562 (PDPA). ราชกิจจานุเบกษา เล่ม 136 ตอนที่ 69 ก.

ด้านเทคโนโลยี

LiveKit Inc. (2024). LiveKit Documentation — Open Source WebRTC Infrastructure. https://docs.livekit.io/
Django Software Foundation. (2024). Django REST Framework Documentation. https://www.django-rest-framework.org/
...
Felix Angelov. (2024). flutter_bloc Package Documentation. https://bloclibrary.dev/
Pezant R., Angelov F. (2024). GoRouter Navigation Documentation. https://pub.dev/packages/go_router
Internet Engineering Task Force. (2011). RFC 6455 — The WebSocket Protocol. IETF.
Internet Engineering Task Force. (2021). RFC 8866 — Session Description Protocol (SDP) — underlying WebRTC signaling.
Caddy Community. (2024). Caddy Documentation. https://caddyserver.com/docs/
Firebase by Google. (2024). Firebase Cloud Messaging Documentation. https://firebase.google.com/docs/cloud-messaging
...
Redis Ltd. (2024). Redis 7 Documentation. https://redis.io/docs/

Appendix A: Ghost Name Generation
...
Appendix C: PHQ-9 Scoring Table
