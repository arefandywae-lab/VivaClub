...
*   **Redis (In-Memory Data Store & Message Broker)**
    *   **มันคืออะไร?:** Redis (Remote Dictionary Server) คือฐานข้อมูลที่บันทึกทุกอย่างลงบน **"หน่วยความจำหลัก (RAM)"** ไม่ได้เขียนลง Harddisk เหมือน Postgres ทำให้มันถูกอ่าน/เขียนด้วยความเร็วระดับไมโครวินาที (เร็วกว่า Database ทั่วไปมหาศาล) แลกกับการที่ถ้าไฟดับข้อมูลใน RAM จะหายไป เราจึงไม่ใช้ Redis เก็บข้อมูลสำคัญถาวร แต่เอามาใช้ทำงานชั่วคราวเป็น **"ผู้ช่วยอเนกประสงค์ (Swiss Army Knife)"**
    *   **ตัวอย่างหน้าที่การทำงานจริงในระบบ 3 ข้อหลัก:**
        1.  **Channel Layer (บุรุษไปรษณีย์ความเร็วสูงสำหรับ WebSockets / Pub-Sub):**
            *   *ปัญหา:* ในห้องเสียงมีคนฟัง 100 คน โปรเซสของ Django (คนรับ API) กับโปรเซสของ Daphne (คนถือสาย WebSocket 100 สาย) มันรันอยู่คนละส่วน คุยกันตรงๆ ไม่ได้
            *   *การแก้ปัญหา (Flow):* เมื่อ User "A" กดไอคอน "ยกมือขอพูด" (ยิง API ไปหา Django) -> Django จะเอาข้อความไปฝาก Redis -> Redis ทำหน้าที่กระจายข่าว (Broadcast) ไปบอก Daphne ทันที -> Daphne ก็จะสั่งหน้าจอมือถืออีก 99 เครื่องในห้องให้โชว์ "ไอคอนยกมือที่โปรไฟล์ A" พร้อมๆ กันทันทีในเสี้ยววินาที!
        2.  **Task Queue (ระบบคิวสั่งงานบอทเบื้องหลัง - Asynchronous Tasks):**
            *   *ปัญหา:* การโยนคำสั่งให้แอคเคาท์ Bot เชื่อมต่อเวที LiveKit เป็นงานที่หนัก อาจใช้เวลาประมวลผล 5-10 วินาที ถ้าให้ API ยืนรอค้างจนบอทเข้าห้องเสร็จ หน้าจอ UI ของผู้ใช้จะค้าง/หมุนโหลดไม่หยุด
            *   *การแก้ปัญหา (Flow):* API จะสร้าง "ใบสั่งงาน (Job Ticket)" เขียนว่า *"บอท 1 ไปเปิดเพลงสตรีมใส่ห้อง B ด่วน"* เอาไปแปะทิ้งไว้ในส่วน Task Queue ของ Redis แล้วตัว API จะตอบกลับ User ทันทีว่า "ส่งคำสั่งสำเร็จ" (UI ไหลลื่นต่อไม่สะดุด) จากนั้น **Bot Worker** ที่รันสแตนด์บายเฝ้า Redis อยู่ตลอดเวลา จะหยิบตั๋วใบนั้นไปทำเชื่อมต่อเข้า LiveKit เงียบๆ แบบเบื้องหลังเอง
        3.  **Caching (ลดภาระ Database ถาวร - Database Load Reduction):**
            *   *ตัวอย่าง:* ยอดคนออนไลน์ (Active Users) หรือสถานะห้อง มีตัวเลขเปลี่ยนไปมาตลอดเวลา ถ้าให้แอปสั่ง Query นับแถวจาก Postgres Database เล่มใหญ่ทุกวินาที เซิร์ฟเวอร์จะรับภาระหนักจนล่ม (Overload) เราจึงเอาตัวเลขพวกนี้เก็บฝากไว้ใน Redis เพราะบวกลบเลขดึงค่าจาก RAM ได้รวดเร็วและใช้ Resource ต่ำกว่ามหาศาล
    *   **ข้อจำกัด (Trade-offs):** ข้อมูลอยู่บน RAM หากเซิร์ฟเวอร์โดนรีสตาร์ทกระทันหัน ข้อมูลคิวงานที่ยังไม่ได้ทำอาจจะหายได้ (แต่ไม่ต้องกังวลเพราะข้อมูลสำคัญถาวรอย่าง Profile User เราแยกเก็บไว้ใน Postgres อย่างปลอดภัยแล้ว)
